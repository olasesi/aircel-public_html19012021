...
class ModelExtensionShippingWeight extends Model {
	public function getQuote($address, $weight = 0) { // optional parameter passed please check below
		...
		foreach ($query->rows as $result) {
			...
			if ($status) {
				$cost = '';
				$weight = $weight != 0 ? $weight : $this->cart->getWeight();  // added by Dominic 16/06/2020 to solve incorrect calculations for mobile
