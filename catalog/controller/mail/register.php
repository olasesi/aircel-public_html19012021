...
class ControllerMailRegister extends Controller {
	public function index(&$route, &$args, &$output) {
		...
		//$mail->AddReplyTo('sales@obejor.com', 'New Registration');
        ...
        $mailb->addAddress($args[0]['email']);
        $mailb->addCC("obejorbusiness@gmail.com");
        $mailb->addBCC("icisystemng@gmail.com");
        $mailb->Subject = sprintf($this->language->get('text_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'));
        $mailb->isHTML(true);
        $mailb->Body = $this->load->view('mail/register', $data);
        $mailb->send();
